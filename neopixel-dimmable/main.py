...
brightness = machine.ADC(27)
color = machine.ADC(26)
...
def Pb_Switch_INT(pin):         # PB_Switch Interrupt handler
    ...
    if (Pb_Switch.value() == 1) and (Pb_Switch_State == 0):  # Pb_Switch is active (High) and Pb_Switch State is currently Low
        ...
        timer.deinit()
        timer.init(period=10000, mode=machine.Timer.ONE_SHOT, callback=mycallback)
        timer2.deinit()
...
Pb_Switch = machine.Pin(16, machine.Pin.IN, machine.Pin.PULL_DOWN)
